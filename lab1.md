...
3. **DevOps / платформенная команда**  
...
4. **Продакт-менеджеры / менеджеры проектов**  
...
5. **Служба информационной безопасности**  
   Отвечает за контроль разграничения доступа, отсутствие утечек чувствительных данных и аудит использования ассистента.
...
4. **Поиск реализаций бизнес-функций**  
   Должна быть возможность искать реализацию функционала по описанию предметной области (например, расчёт тарифа, обработка платежа) и получать релевантные участки кода и документации.
...
10. **Интерфейсы взаимодействия**  
    Система должна иметь веб-интерфейс (чат) и программный API для интеграции с IDE, мессенджерами и CI-системами.
...
1. Монорепозиторий хранится в Git и использует типовой процесс разработки (ветки, merge-/pull-реквест
ы).  
